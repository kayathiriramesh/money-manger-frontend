...
function Expenseedit() {
    const notify = () => toast.success('Expense edited successfully', {
        position: "top-right",
        autoClose: 5000,
    ...
    useEffect(()=>{
      ...
      let fetchdata=async()=>{
        let a=await axios.get(`${config.api}/portal/expense/${params.id}`, {
            headers: {
    ...
    let formik=useFormik({
        ...
        onSubmit:async (values)=>{
            try {
                delete values._id;
                await axios.put(`${config.api}/portal/expense/edit/${params.id}`, values, {
                    headers: {
                      //'authorization': `${localStorage.getItem('react_app_token')}`
                      'authorization':`Bearer ${JSON.parse(window.localStorage.getItem("token"))}`
                    }
